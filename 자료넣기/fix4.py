    ...
    def add_red_line():
        ...
        try:
            ...
            for line in ax.lines:
                if line.get_color() == 'r':  # 경고선은 빨간색이므로, 빨간선만 삭제
                    line.remove()

            # 새로운 경고선 그리기
            xlim = ax.get_xlim()  # 기존 x축 범위

            ax.plot([xlim[0], xlim[1]],  # x축 범위
                    [set_money_value, set_money_value],  # y축 값 (경고 요금)
                    'r--')  # 빨간색 점선
            ax.set_xlim(xlim)  # x축 범위 고정
